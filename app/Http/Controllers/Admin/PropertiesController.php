...
class PropertiesController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request('show_deleted') == 1) {
            ...
            $properties = auth()->user()->role->contains('id', 2) ? Property::where('user_id', auth()->user()->id)->get() : BasicSvc::getTenantProperties();
        }
    ...
    public function show($id)
    {
        ...
        $notes     = Note::where('property_id', $id)
        ->where(function($q)use($property){
            return $q->where('user_id', $property->user_id)->orWhere('user_id', auth()->user()->id);
        })
        ->get();
