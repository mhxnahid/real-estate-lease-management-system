                    ...
                    @if (count($tenants) > 0)
                        @foreach ($tenants as $tenant)
                            <tr data-entry-id="{{ $tenant->id }}">
                                <td field-key='name'>{{ $tenant->tenant->name }}</td>
                                <td field-key='email'>{{ $tenant->tenant->email }}</td>
                                <td field-key='accepted_invite'>{{ $tenant->accepted_invite ? 'Yes' : 'No' }}</td>
                                <td field-key='active'>{{ $tenant->active ? 'Yes' : 'No' }}</td>
                                <td>
                                    @can('property_edit')
